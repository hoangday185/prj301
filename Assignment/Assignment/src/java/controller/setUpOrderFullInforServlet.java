 ...
 * @author hoanghamhoc
 */
public class setUpOrderFullInforServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            FullInforOrder setUpFullInforOrder = null;
            for (FullInforOrder fullInforOrder : listFullInforOrder) {
                if (fullInforOrder.getOrder().getOrderID().compareTo(orderId) == 0) {
                    setUpFullInforOrder = fullInforOrder;
                    break;
                }
            }

            ArrayList<Shipper> listShipper = ShipperDAO.getListShipper();
            request.setAttribute("listShipper", listShipper);
            request.setAttribute("setUpFullInforOrder", setUpFullInforOrder);
