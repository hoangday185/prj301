...
public class showDetailOrderServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            ArrayList<FullInforOrder> listFullInforOrder = (ArrayList<FullInforOrder>) request.getSession().getAttribute("listFullOrder");
            FullInforOrder readFullInforOrder = null;
            for (FullInforOrder fullInforOrder : listFullInforOrder) {
                if (fullInforOrder.getOrder().getOrderID().compareTo(orderId) == 0) {
                    readFullInforOrder = fullInforOrder;
                    break;
                }
            }
            request.setAttribute("readFullInforOrder", readFullInforOrder);
